...
def juego_ahorcado():
    ...
    def verificar():
        ...
        if '_' not in mostrar():
            ...
            mensaje.set(f"Perdiste. Era: {palabra}")
            boton.config(state=tk.DISABLED)
...
def juego_snake():
    ...
    class Snake:
        def __init__(self, ventana):  # CORREGIDO: se añadió 'self'
        ...
        def mover(self):
            ...
            if nueva in self.snake or x < 0 or y < 0 or x >= ANCHO or y >= ALTO:
                self.canvas.create_text(200, 200, text="Fin del juego", fill="red", font=("Arial", 24))
                return
    ...
    Snake(ventana)  # Llama al juego correctamente
# =============== JUEGO 4: Pong ===============
def juego_pong():
    ocultar_ventana_principal()
    v = tk.Toplevel()
